...
//    
    ...
    public UsuariosEntidad buscarPorCorreoYIdSin(UsuariosEntidad usuario) {
        return usuariosRepositorio.findByCorreoAndUsuarioIDNotOrUsuarioIDNot(usuario.getCorreo(), usuario.getUsuarioID(), usuario.getUsuarioID());
        /*
    En este caso, se está utilizando el método findByCorreoAndUsuarioIDNotOrUsuarioIDNot, que incluye dos condiciones en la consulta:
    - La primera condición busca entidades por correo electrónico y excluye la entidad con el ID especificado (UsuarioIDNot).
    - La segunda condición excluye la entidad con el ID especificado (UsuarioIDNot), de manera que se obtiene la entidad con un correo electrónico 
      diferente al especificado y con un ID diferente al que se está editando.

    Al incluir ambas condiciones con el operador "OR", se obtiene la entidad que cumple cualquiera de las dos condiciones.

    Es importante mencionar que esta convención de nomenclatura de Spring Data JPA solo funciona si la entidad tiene un 
    atributo id (o un atributo con un nombre diferente, pero que se haya especificado con la anotación @Id), ya que se basa 
    en la convención de que el identificador de la entidad es un atributo llamado "id". Si la entidad no tiene un atributo id, se debe 
    utilizar otra convención de nomenclatura para indicar el atributo que se utilizará como identificador en la consulta.
         */
    }
    ...
    public UsuariosEntidad buscarPorCorreoSin(UsuariosEntidad usuario) {
        ...
        /*
        En este caso, el método que se está utilizando es findByCorreoAndUsuarioIDNot, que sigue la convención de nomenclatura 
        de Spring Data JPA. El "IDNot" en este caso indica que se buscará una entidad por correo electrónico (findByCorreo) y 
        que se excluirá la entidad con el ID especificado (UsuarioIDNot). De esta manera, se obtendrá la entidad que tenga el correo electrónico 
    ...
    public UsuariosEntidad buscarPorIdSin(UsuariosEntidad usuario) {
        ...
        /*
    En este caso, el método que se está utilizando es findByUsuarioIDNot, que sigue la convención de nomenclatura 
    de Spring Data JPA. El "IDNot" en este caso indica que se buscará una entidad excluyendo la entidad con el ID especificado. 
    De esta manera, se obtendrá la entidad que tenga un ID diferente al que se está editando.
